...
<% 
	...
	if ("true".equals(search)) {
		totalArticle = VcmsArticleLocalServiceUtil.countArticleByParam(groupId, PortalConstant.DEFAULT_LANGUAGE, statusArticle,
				categoryId, typeId, keyword, isTitle, isLead, isContent, searchBy, isFromTo, searchFrom, searchTo, tabs, true);
		
		listArticle = VcmsArticleLocalServiceUtil.findArticleByParam(groupId, PortalConstant.DEFAULT_LANGUAGE, statusArticle,
				categoryId, typeId, keyword, isTitle, isLead, isContent, searchBy, isFromTo, searchFrom, searchTo, tabs, true, begin, end);
	} else {
...
<aui:form action="<%=searchArticletURL.toString()%>" method="post" name="fm" id="fm"> 
...
<liferay-ui:search-container-results 
	...
	<liferay-ui:search-container-row className="vn.gov.hoabinh.model.VcmsArticle" modelVar="article" keyProperty="articleId" >
   	...
   	<%
   		...
   		sb.append("<div>");
   		sb.append("<b>"+article.getTitle()+"</b><br/>");
   		// sb.append("<div>");
   		// sb.append(article.getSticky()?"<img id='"+ article.getArticleId() +"' src='"+img1+"' />":"&nbsp;");   		
   		// sb.append(article.getDiscussible()?"<img id='"+ article.getArticleId() +"' src='"+img2+"' />":"&nbsp;");
   		// sb.append(article.getHasPoll()?"<img id='"+ article.getArticleId() +"' src='"+img3+"' />":"&nbsp;");
   		sb.append("</div>");
   	%>
   	...
   	<liferay-ui:search-container-column-text href="<%=editURL %>" name="title" value="<%=sb.toString() %>"/>
...
Liferay.provide(
        ...
        function() {            	        
                    ...
                    if (confirm('Bạn thực sự muốn thực hiện hành động với những bài viết đã lựa chọn?')) {
                    	...
                    	document.<portlet:namespace/>fm.action = "<%=searchArticletURL%>";
